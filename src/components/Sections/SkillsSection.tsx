...
function SkillsSectionMemo({ className }: { className?: string }) {
  return (
    <Section className={cn("flex-col-center gap-4", className)}>
      <H2 className={`font-bold ${playFair.className}`}>Skills</H2>
...
function SkillCard({
  ...
  return (
    <motion.div
      className={cn(
        "flex-col-center place-content-start gap-2 rounded-lg bg-white p-4 shadow-lg",
        className,
      )}
      whileHover={{ scale: 1.05 }}
      transition={{ type: "spring", stiffness: 300 }}
    >
      <h3 className={`text-xl font-extrabold ${playFair.className}`}>
        {title}
      </h3>
